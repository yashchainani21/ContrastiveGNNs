...
PROJ_DIM = 256
SEED = 42
SUBSET_SIZE = 1000 # total number of datapoints to train on
...
def evaluate_linear_probe(model, dataloader, device, max_samples=5000):
    ...
    with torch.no_grad():
        for xb, yb in dataloader:
            ...
            g, z = model(xb)  # use g (encoder output) or z (projection)
...
# ---- sampler to balance within the subset ----
...
sampler = WeightedRandomSampler(
    ...
    num_samples=len(train_ds),  # draw as many samples as subset size
    replacement=True
)
